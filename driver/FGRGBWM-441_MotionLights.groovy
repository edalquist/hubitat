...
private onChX(channel) {
  ...
  if (channel < 1 || channel > 4 ) {
    ...
    cmds << zwave.multiChannelV3.multiChannelCmdEncap(destinationEndPoint: (channel + 1) )
      .encapsulate(zwave.switchMultilevelV1.switchMultilevelSet(value: newLevel.toInteger())).format() // Endpoint = channel + 1
...
private setLevelChX(level, channel) {
  ...
  if (channel > 4 || channel < 1 ) {
    ...
    cmds << zwave.multiChannelV3.multiChannelCmdEncap(destinationEndPoint: (channel + 1) ).encapsulate(zwave.switchMultilevelV1.switchMultilevelSet(value: level.toInteger())).format() // Endpoint = channel + 1
    sendEvent(name: "savedLevelCh${channel}", value: null) // Wipe savedLevel.
    sendEvent(name: "activeProgram", value: 0) // Wipe activeProgram.
  }
